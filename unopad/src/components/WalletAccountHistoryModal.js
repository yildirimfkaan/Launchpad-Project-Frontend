...
import { Button, Col, Container, Row } from 'react-bootstrap';
 ...
 function walletAccountHistoryModal({...props}) {
  const {walletAccountHistoryModal,walletAccountHistory,walletAccountHistoryModalRequest} = props;
      ...
      <Modal show={walletAccountHistoryModal} fullscreen={true} onHide={handleClose}>
        ...
        {walletAccountHistory?.map((item, index) => (
               ...
               <tr>
                 ...
                 <td>{item.from.slice(0, 10)}...{item.from.slice(30, 42)}</td>
                 <td>{item.to.slice(0, 10)}...{item.to.slice(30, 42)}</td>
                 <td>{item.value}</td>
                 <td> <Button href={'https://testnet.bscscan.com/tx/' + item.hash} target="_blank" variant="info"> {item.hash.slice(0, 10)} </Button></td>
