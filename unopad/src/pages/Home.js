...
class Home extends React.Component {
  ...
  componentDidMount() {
    setTimeout(() => {
      fetch(process.env.REACT_APP_API_URL+'/api/projects').then(response => response.json()).then(data => this.setState({ data, isLoading:false, status:"Completed" }));
  ...
  return (
    ...
    <br></br>
    {this.state.isLoading ? <h1>Page is Loading.....</h1> :<><Card1{...this.state.data} /> </>}
    <br></br>
    <React.Fragment>
        <About/>
        <Service/>
